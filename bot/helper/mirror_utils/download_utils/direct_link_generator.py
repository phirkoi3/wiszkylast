...
def fichier(link: str) -> str:
    ...
    if not gan:
        raise DirectDownloadLinkException("ERROR: You Insert Wrong Link!")
    if "::" in link:
        pswd = link.split("::")[-1]
        url = link.split("::")[-2]
    else:
    ...
    try:
        ...
        raise DirectDownloadLinkException("ERROR: Server 1fichier going down!")
    ...
    if soup.find("a", {"class": "ok btn-general btn-orange"}) is not None:
        ...
        if dl_url is None:
            raise DirectDownloadLinkException("ERROR: Can't get direct link from 1fichier!")
        else:
            return dl_url
        ...
        if "you must wait" in str(str_2).lower():
            ...
            if not numbers:
                raise DirectDownloadLinkException("ERROR: 1fichier is being limited please wait some minutes.")
            else:
                raise DirectDownloadLinkException(f"ERROR: 1fichier is being limited, please wait {numbers[0]} minutes.")
        elif "protect access" in str(str_2).lower():
            raise DirectDownloadLinkException(f"ERROR: This Link required password!\n\n- Add sign <b>::</b> after link and type password after that sign.\n\n<b>Example:</b>\n<code>/{BotCommands.MirrorCommand[0]} https://1fichier.com/?smmtd8twfpm66awbqz04::love you</code>\n\n* Without space between link and password <b>::</b>\n* But password could use space")
        else:
            raise DirectDownloadLinkException("ERROR: Failed while generate direct link from 1fichier!")
        ...
        if "you must wait" in str(str_1).lower():
            ...
            if not numbers:
                raise DirectDownloadLinkException("ERROR: 1fichier been limited, please wait some minutes.")
            else:
                raise DirectDownloadLinkException(f"ERROR: 1fichier been limited, Please wait {numbers[0]} minutes.")
        elif "bad password" in str(str_3).lower():
            raise DirectDownloadLinkException("ERROR: You Enter Wrong Password!")
        else:
            raise DirectDownloadLinkException("ERROR: Failed while generate direct link from 1fichier!")
    else:
        raise DirectDownloadLinkException("ERROR: Failed while generate direct link from 1fichier!")
