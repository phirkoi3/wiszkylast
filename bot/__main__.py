...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("📢 Source Code", 'https://github.com/junedkh/jmdkh-mltb')
    reply_markup = buttons.build_menu(1)
    uname = mention_html(update.message.from_user.id, update.message.from_user.first_name)
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f' Hi <b>{uname}</b>\n\n' \
        '🌹 Welcome To MIRRORIN\n' \
        'This bot can Mirror all your links To Google Drive and Telegram!\n' \
        '👨🏽‍💻 @mirrorin Group Powered By: @JMDKH_Team'
